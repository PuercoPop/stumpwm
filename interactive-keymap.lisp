...
(defmacro define-interactive-keymap (name (&key on-enter on-exit abort-if on-abort) &body key-bindings)
    ...
    `(let ((,keymap (make-sparse-keymap)))
       ...
       (defcommand ,name () ()
         ...
         ,@(when abort-if `((when (funcall ,abort-if)
                              ,@(when on-abort (list on-abort))
                              (return-from ,command))))

         ,@(when on-enter `((funcall ,on-enter)))
         (enter-interactive-keymap ,keymap ',command))

       (defcommand ,(intern exit-command) () ()
         ,@(when on-exit `((funcall ,on-exit)))
         (exit-interactive-keymap ',command)))))
